...
```{r}
scatter_df <- scatter_df %>% 
  mutate(emissions_pc = (total_emissions / population) * 1000000) %>% # create 'emissions_pc' column
...
# obtain main 6 countries and select relevant columns
...
#country_colors <- c("U.S." = "#020122", "Brazil" = "#020122", "Russia" = "#020122", "Canada" = "#020122", "EU*" = "#020122", "China" = "#020122", "Australia" = "#020122")
...
scatter_df$alpha_value <- ifelse(scatter_df$country %in% c("China", "U.S.", "Russia", "Brazil", "Australia", "Canada", "EU*"), 1, 0.2)

# Now, in your ggplot code, use this new column for the alpha aesthetic
ggplot(scatter_df) +
  ...
  # Your other layers and customizations follow...
  geom_text(data = australia_df, aes(label = country, x = population - 5, y = emissions_pc + 15), 
            size = 5, hjust = 0, family = "barlow", fontface = "bold", check_overlap = TRUE) +
  geom_text(data = other_main_countries, aes(label = country, x = population + 15, y = emissions_pc), 
            size = 5, hjust = 0, family = "barlow", fontface = "bold", check_overlap = TRUE) +
  # geom_text(data = russia_df, aes(label = country, x = population + 15, y = emissions_pc), 
  #           size = 5, hjust = 0, family = "barlow", fontface = "bold", check_overlap = TRUE) +
  geom_text(data = other_notable_countries, aes(label = country, x = population + 15, y = emissions_pc), alpha = 0.2, size = 5, hjust = 0, family = "barlow", fontface = "bold", check_overlap = TRUE) +
  #geom_point(data = africa_df, aes(x = population, y = emissions_pc), color = "#C23E37") +
  #geom_text(data = africa_df, aes(label = country, x = population + 20, y = emissions_pc),
             # color = "#C23E37", size = 5, hjust = 0, vjust = 0, family = "barlow", fontface = "bold", check_overlap = TRUE) +
  labs(x = "Population (millions)", y = "Per Capita Methane Emissions (tons CO2eq)", 
       title = "Population and Per-Capita Anthropogenic Methane Emissions in 2021",
       #subtitle = "Other countries shown with grey points",
       caption = "Note: 8 countries had per capita emissions >350 tons CO2eq and are not displayed.\nData Sources: International Energy Agency (IEA), World Bank\n\n*The EU is a group of 27 countries in Europe.\n**Based on calculation from BBC News in 2021 article.⁵") + # Add label for size legend
  scale_x_continuous(limits = c(0, 1550), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 350), expand = c(0, 0)) +
  theme_minimal() +
  theme(panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(),
        axis.title.x = element_text(family = "barlow", face = "bold", size = 15, color = "grey30", margin = margin(20, 0, 0, 0)),
        axis.title.y = element_text(family = "barlow", face = "bold", size = 15, color = "grey30", margin = margin(0, 20, 0, 0)),
        ...
        axis.text.x = element_text(family = "barlow", face = "bold", size = 13),
        axis.text.y = element_text(family = "barlow", face = "bold", size = 13),
        axis.line = element_line(color = "black", size = 0.5),
        plot.title = element_text(family = "merri sans", size = 12.5, hjust = 0.5),
        #plot.subtitle = element_text(family = "merri sans", size = 10),
        plot.caption = element_text(hjust = 1, size = 8, color = "grey30", family = "merri sans", margin = margin(20, 0, 0, 0)),
        plot.background = element_rect(fill = "#FEF6EC", color = NA),
        panel.background = element_rect(fill = "#FEF6EC", color = NA)) +
  geom_hline(yintercept = 197, linetype = "dashed", linewidth = 0.8, color = "cornflowerblue") + # Add horizontal line at y = 1
  annotate("text", x = 700, y = 240, label = "Carbon footprint of a typical\nprivate jet flying for 48 hours**", size = 5, hjust = 0, color = "cornflowerblue", family = "barlow", fontface = "bold")
```
...
ggplot(cols_df, aes(x = country, y = total_emissions, fill = type)) + # fill columns based on sector
  geom_col(position = "stack") + # specify dodged position so to create space between countries
  labs(x = "", y = "Methane Emissions (million tons CO2eq)", title = "Sources of 2021 Anthropogenic Methane Emissions in selected countries", caption = "Data Source: International Energy Agency (IEA)\n\n*The EU is a group of 27 countries in Europe.") + # add source as x axis title
  scale_fill_manual(values = custom_colors) + # apply custom color palette
  theme_minimal() +
  theme(panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(),
        panel.grid.major.y = element_line(color = "grey90", size = 0.5), # Add horizontal gridlines (major)
        panel.grid.minor.y = element_line(color = "grey90", size = 0.25), # Add horizontal gridlines 
        plot.caption = element_text(size = 8, hjust = 1, colour = "grey30", family = "merri sans", margin = margin(20, 0, 0, 0)), # adjust font, fontface, size, and color of x axis title (use hjust = 1 to move to far right since this is source)
        axis.title.y = element_text(family = "barlow", face = "bold", size = 15, color = "grey30", margin = margin(0, 20, 0, 0)), # adjust font, fontface, size, and color of y axis title
        axis.text.x = element_text(family = "barlow", face = "bold", size = 16), # adjust font and fontface of x axis text
        axis.text.y = element_text(family = "barlow", size = 13, face = "bold"), # adjust font and fontface of y axis text
        legend.position = c(0.88, 0.85), # specify legend position
        legend.text = element_text(color = "grey30", face = "bold", size = 16, family = "barlow"), # adjust font, fontface, size, and color of y axis legend title
        legend.title = element_blank(),
        plot.title = element_text(family = "merri sans", size = 12.5, hjust = 0.5),
        legend.background = element_rect(fill = "#FEF6EC", color = NA),
        plot.background = element_rect(fill = "#FEF6EC", color = NA), # Change the plot background color
        panel.background = element_rect(fill = "#FEF6EC", color = NA)) # Change the panel background color to)
```
